...
if __name__ == '__main__':
    ...
    cancel_handler = CommandHandler('cancel', cancel)  # поправка Р.Тюрин (ниже добавлен: cancel_handler аргументом во все строки)
    '''
    ...
    conv_handler = ConversationHandler( # здесь строится логика разговора
        ...
        entry_points=[CommandHandler('start', start), cancel_handler],
        # этапы разговора, каждый со своим списком обработчиков сообщений
        states={
            ENTER_NUMBER_ONE: [MessageHandler(Filters.regex('^(рациональное|комплексное)$'), enter_number_one), cancel_handler],
            RATIONAL_ONE: [MessageHandler(Filters.text, rational_one), cancel_handler],
            COMPLEX_ONE: [MessageHandler(Filters.text, complex_one), cancel_handler],
            ENTER_NUMBER_TWO: [MessageHandler(Filters.all, enter_number_two), cancel_handler],
            RATIONAL_TWO: [MessageHandler(Filters.text, rational_two), cancel_handler],
            COMPLEX_TWO: [MessageHandler(Filters.text, complex_two), cancel_handler],
            CALCULATIONS: [MessageHandler(Filters.text, calculations), cancel_handler],
